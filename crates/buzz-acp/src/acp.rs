...
fn permission_response_for_request(message: &serde_json::Value) -> Option<serde_json::Value> {
    ...
    let options = message
        ...
        .unwrap_or_default();
    Some(
        permission_denial_response(&id, options).unwrap_or_else(|error| {
            serde_json::json!({
                "jsonrpc": "2.0",
                "id": id,
...
impl AcpClient {
    ...
    pub fn take_turn_usage(&mut self) -> Option<TurnUsage> {
        let goose_usage = self.usage_session.take().and_then(|session_id| {
            self.shared
                .goose_usage
                .lock()
                .ok()
    ...
    pub(crate) fn notify_session_spawned(&mut self, session_id: &str) {
        if let Ok(mut usage) = self.shared.goose_usage.lock() {
            usage.seed_zero_baseline(session_id);
        }
...
mod tests {
    ...
    async fn spawn_named_script(name: &str, script: &str) -> (AcpClient, std::path::PathBuf) {
        ...
        panic!("spawn named fake adapter: {last_error:?}")
    }
    ...
    #[tokio::test]
    async fn shared_reader_rejects_permission_once_with_advertised_option() {
        let script = r#"
            IFS= read -r _prompt
            printf '%s\n' '{"jsonrpc":"2.0","id":"perm-a","method":"session/request_permission","params":{"sessionId":"session-a","options":[{"optionId":"dynamic-allow","kind":"allow_once"},{"optionId":"dynamic-reject","kind":"reject_once"}]}}'
            IFS= read -r permission
            case "$permission" in
              *'"id":"perm-a"'*'"optionId":"dynamic-reject"'*)
                printf '%s\n' '{"jsonrpc":"2.0","id":0,"result":{"stopReason":"end_turn"}}'
                ;;
              *)
    ...
    async fn goose_usage_stays_exclusive_and_drains_standard_usage() {
        ...
        client
            ...
            .lock()
            .unwrap()
            .begin_turn("goose-session");
        client.usage_session = Some("goose-session".into());
