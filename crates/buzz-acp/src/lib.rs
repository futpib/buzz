...
async fn handle_switch_model_control(
    ...
    let status = if pool.channel_control_is_ambiguous(channel_id) {
        ...
        // Idle path: validate against the cached catalog before invalidating.
        match pool
            .switch_idle_agent_model(channel_id, model_id, request_id.clone())
            .await
        {
                ...
                buzz_event = relay.next_event() => {
                    ...
                    match buzz_event {
                        Some(buzz_event) => {
                            ...
                            // Resolve channel type once for control scoping,
                            // author hardening, and normal conversation
                            // routing. Resolution fails closed to DM.
                            let is_dm =
...
fn dispatch_pending(
    ...
    loop {
        ...
        dispatched_scopes.push((scope, typing_scope));
        *last_activity = tokio::time::Instant::now();
    }
...
async fn handle_prompt_result(
    ...
    let successful_steer_deliveries = pool
        .task_map()
        .values()
        .find(|meta| meta.turn_id == result.turn_id)
    ...
    if let PromptSource::Channel(scope) = &result.source {
        result
            .agent
            .record_successful_steers(scope, successful_steer_deliveries);
    }
    ...
    for ch in removed_channels {
        let channel_still_active = pool.task_map().values().any(|meta| {
            meta.scope
                .as_ref()
                .is_some_and(|scope| scope.channel_id() == *ch)
        });
    ...
    if let Some(ref observer) = observer {
        observer.emit(
            ...
            &observer::context_for(
                meta.scope.as_ref().map(scope::SessionScope::channel_id),
                None,
                Some(meta.turn_id),
            ),
            serde_json::json!({
...
mod error_outcome_emission_tests {
    ...
    async fn successful_native_steer_is_transferred_to_live_session_delivery_state() {
        ...
        agent.test_set_session(scope.clone(), "live-session");

        let mut pool = AgentPool::from_slots(vec![Some(agent.clone())]);
        ...
        assert!(returned.test_delivery_contains(&scope, steer_event_id));
    }
    ...
    async fn in_flight_stale_native_steer_ack_cannot_update_replacement_session() {
        ...
        let scope = scope::SessionScope::Conversation { channel_id };
        let mut agent = dummy_agent(0).await;
        agent.test_set_session(scope.clone(), "replacement-session");
        ...
        let returned = pool.agents_mut()[0].as_ref().expect("returned agent");
        assert!(!returned.test_delivery_contains(&scope, "stale-event"));
    }
    ...
    async fn invalidated_session_does_not_resurrect_successful_steer_delivery_state() {
        ...
        let returned = pool.agents_mut()[0].as_ref().expect("returned agent");
        assert!(!returned.test_has_delivery_state(&scope));
    }
    ...
    async fn indeterminate_project_context_requeues_without_poisoning_agent_or_circuit() {
        ...
        let mut agent = dummy_agent(0).await;
        agent.test_set_session(session_scope.clone(), "healthy-session");
        let mut pool = AgentPool::from_slots(vec![None]);
        let task_id = pool.join_set.spawn(async {}).id();
        pool.task_map_mut().insert(
            task_id,
        ...
        assert_eq!(
            returned.test_session_id(&session_scope).as_deref(),
