                ...
                buzz_event = relay.next_event() => {
                    ...
                    match buzz_event {
                        Some(buzz_event) => {
                            ...
                            if is_cancel {
                                if let Some(owner) = owner_cache.get() {
                                    if buzz_event.event.pubkey.to_hex() == *owner {
                                        ...
                                        let key = ConversationKey::for_control(
                            ...
                            let conversation = ConversationKey::for_event(
                                buzz_event.channel_id,
                                &buzz_event.event,
                                is_dm,
                            );
...
async fn handle_prompt_result(
    ...
    if let PromptSource::Channel(conversation) = &result.source {
        result
            .agent
            .record_successful_steers(conversation, successful_steer_deliveries);
    }
...
mod error_outcome_emission_tests {
    ...
    async fn in_flight_stale_native_steer_ack_cannot_update_replacement_session() {
        ...
        let agent = dummy_agent(0).await;
        agent.test_set_session(conversation.clone(), "replacement-session");
        ...
        let returned = pool.agents_mut()[0].as_ref().expect("returned agent");
        assert!(!returned.test_delivery_contains(&conversation, "stale-event"));
    }
    ...
    async fn invalidated_session_does_not_resurrect_successful_steer_delivery_state() {
        ...
        let returned = pool.agents_mut()[0].as_ref().expect("returned agent");
        assert!(!returned.test_has_delivery_state(&conversation));
    }
    ...
    async fn indeterminate_project_context_requeues_without_poisoning_agent_or_circuit() {
        ...
        let agent = dummy_agent(0).await;
        agent.test_set_session(conversation.clone(), "healthy-session");
        let mut pool = AgentPool::from_slots(vec![Some(agent.clone())]);
        let task_id = pool.join_set.spawn(async {}).id();
        pool.task_map_mut().insert(
            task_id,
        ...
        assert_eq!(
            returned.test_session_id(&conversation).as_deref(),
