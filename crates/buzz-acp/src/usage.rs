...
pub(crate) struct UsageTracker {
    ...
    /// Per-session state for prompts multiplexed over the shared connection.
    in_flight: HashMap<String, InFlightUsage>,
}
...
impl UsageTracker {
    ...
    pub(crate) fn begin_turn(&mut self, session_id: &str) {
        if let Some(previous) = self.in_flight.remove(session_id) {
            self.commit_absence(
                session_id,
                previous.input_absence_observed,
                previous.output_absence_observed,
            );
        }
        self.in_flight
            .insert(session_id.to_string(), InFlightUsage::default());
    }
    ...
    pub(crate) fn record(&mut self, session_id: &str, payload: &UsageUpdatePayload) {
        ...
        let is_in_flight = self.in_flight.contains_key(session_id);
        ...
        if let Some(in_flight) = self.in_flight.get_mut(session_id) {
            if current_input.is_none() {
                in_flight.input_absence_observed = true;
            }
            if current_output.is_none() {
                in_flight.output_absence_observed = true;
            }
        }

        let (input_absence_observed, output_absence_observed) = self
            .in_flight
            .get(session_id)
            .map(|state| (state.input_absence_observed, state.output_absence_observed))
            .unwrap_or_default();
        ...
        let (delta_reliable, turn_input, turn_output, turn_cost, turn_seq) =
            match self.sessions.get(session_id) {
                ...
                Some(prev) => {
                    ...
                    let input_poisoned =
                        prev.input_ever_poisoned || input_absence_observed || this_input_absent;
                    let output_poisoned =
                        prev.output_ever_poisoned || output_absence_observed || this_output_absent;
        ...
        if is_in_flight {
            ...
            let in_flight = self
                .in_flight
                .get_mut(session_id)
                .expect("in-flight session disappeared");
            in_flight.pending_identity = match in_flight.pending_identity.take() {
            ...
            in_flight.pending = Some(TurnUsage {
    ...
    fn commit_absence(&mut self, session_id: &str, input_absent: bool, output_absent: bool) {
        if !input_absent && !output_absent {
            return;
        }
        let existing = self.sessions.get(session_id);
        let state = SessionState {
            published_seq: existing.map_or(0, |state| state.published_seq),
            last_input: existing.and_then(|state| state.last_input),
            last_output: existing.and_then(|state| state.last_output),
            last_cost: existing.and_then(|state| state.last_cost),
            last_total: existing.and_then(|state| state.last_total),
            last_cached_input: existing.and_then(|state| state.last_cached_input),
            last_cache_write: existing.and_then(|state| state.last_cache_write),
            input_ever_poisoned: existing.is_some_and(|state| state.input_ever_poisoned)
                || input_absent,
            output_ever_poisoned: existing.is_some_and(|state| state.output_ever_poisoned)
                || output_absent,
        };
    ...
    pub(crate) fn take(&mut self) -> Option<TurnUsage> {
        let session_id = self.in_flight.keys().next()?.clone();
    ...
    pub(crate) fn take_for(&mut self, session_id: &str) -> Option<TurnUsage> {
        ...
        let folded_identity = in_flight.pending_identity.and_then(|inner| inner);
        let input_absence_this_turn = in_flight.input_absence_observed;
        let output_absence_this_turn = in_flight.output_absence_observed;
        let Some(mut record) = in_flight.pending else {
            self.commit_absence(
                session_id,
