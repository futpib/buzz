//! Agent pool — leases concurrent sessions from shared ACP connections.
//!
//! # Mental model
//!
//! ```text
//!   AgentPool
//!   ├── agents: Vec<Option<OwnedAgent>>   ← live connection templates
//!   ├── active: Vec<usize>                ← logical session leases
...
//!
//!   Dispatch:
//!     try_claim() → OwnedAgent (clone sharing one connection/state)
...
//!     return_agent(agent) → releases one logical lease
//! ```
//!
//! `AcpClient` clones share the process transport; turn-local state stays on the clone.
...
impl SessionState {
    ...
    pub fn invalidate(&mut self, source: &PromptSource) -> Option<String> {
        match source {
            PromptSource::Channel(scope) => self.invalidate_scope(scope),
    ...
    pub fn invalidate_scope(&mut self, scope: &SessionScope) -> Option<String> {
    ...
    pub fn invalidate_channel(&mut self, channel_id: &Uuid) -> Vec<String> {
        ...
        for scope in scopes {
            if let Some(session_id) = self.invalidate_scope(&scope) {
...
/// Package name supporting `_meta.systemPrompt` on `session/new`.
...
impl OwnedAgent {
    pub fn new(
        ...
        Self {
            index,
            acp,
            shared: Arc::new(Mutex::new(AgentRuntimeState {
    ...
    fn runtime(&self) -> std::sync::MutexGuard<'_, AgentRuntimeState> {
    ...
    pub(crate) fn record_successful_steers(
        &mut self,
        scope: &SessionScope,
        deliveries: HashSet<SuccessfulSteerDelivery>,
    ) {
        let mut runtime = self.runtime();
        let Some(live_session_id) = runtime.sessions.sessions.get(scope).cloned() else {
            return;
        };
        ...
        runtime
            .sessions
            .mark_scope_delivery_success(scope.clone(), false, event_ids);
    }

    #[cfg(test)]
    pub(crate) fn test_set_session(&self, scope: SessionScope, session_id: &str) {
        let mut runtime = self.runtime();
        runtime
            .sessions
            .sessions
            .insert(scope.clone(), session_id.to_string());
        runtime
            .sessions
            .deliveries
            .insert(scope, ChannelDeliveryState::default());
    }

    #[cfg(test)]
    pub(crate) fn test_session_id(&self, scope: &SessionScope) -> Option<String> {
        self.runtime().sessions.sessions.get(scope).cloned()
    }

    #[cfg(test)]
    pub(crate) fn test_delivery_contains(&self, scope: &SessionScope, event_id: &str) -> bool {
        self.runtime()
            .sessions
            .deliveries
            .get(scope)
            .is_some_and(|delivery| delivery.delivered_event_ids.contains(event_id))
    }

    #[cfg(test)]
    pub(crate) fn test_has_delivery_state(&self, scope: &SessionScope) -> bool {
        self.runtime().sessions.deliveries.contains_key(scope)
    }
    ...
    pub(crate) fn has_system_prompt_support(&self) -> bool {
        let goose_system_prompt_supported = self.runtime().goose_system_prompt_supported;
        has_system_prompt_support(
            self.protocol_version,
            &self.agent_name,
            goose_system_prompt_supported,
        )
    }
}

/// Pool of process templates with bounded logical session leases.
...
impl AgentPool {
    ...
    pub fn set_session_concurrency(&mut self, concurrency: usize) {
    ...
    pub fn try_claim(&mut self, scope: Option<&SessionScope>) -> Option<OwnedAgent> {
        ...
        if let Some(scope) = scope {
            ...
            if let Some(i) = idx {
                self.active[i] += 1;
                return self.agents[i].clone();
            }
        }

        // Pass 3: first idle agent.
        let idx = self.agents.iter().enumerate().position(|(index, slot)| {
            slot.is_some() && self.active[index] < self.session_concurrency
        });
        idx.and_then(|i| {
            self.active[i] += 1;
            self.agents[i].clone()
    ...
    pub fn return_agent(&mut self, agent: OwnedAgent) {
        ...
        let is_current = self
            ...
            .is_some_and(|current| current.acp.shares_connection_with(&agent.acp));
        if is_current {
            if let Some(active) = self.active.get_mut(idx) {
                *active = active.saturating_sub(1);
            }
        }
    }

    pub fn replace_agent(&mut self, agent: OwnedAgent) {
    ...
    pub fn retire_agent(&mut self, failed: &OwnedAgent) {
        let index = failed.index;
        if let Some(slot) = self.agents.get_mut(index) {
            let is_current = slot
                .as_ref()
                .is_some_and(|current| current.acp.shares_connection_with(&failed.acp));
            if is_current {
                *slot = None;
                self.active[index] = 0;
            }
    ...
    pub fn take_agent(&mut self, index: usize) -> Option<OwnedAgent> {
    ...
    pub fn slot_alive(&self, index: usize) -> bool {
        self.agents.get(index).is_some_and(|slot| slot.is_some())
    }
    ...
    pub async fn invalidate_scope_session(&mut self, scope: &SessionScope) -> usize {
        ...
        for slot in &mut self.agents {
            if let Some(agent) = slot.as_mut() {
                let session_id = agent.runtime().sessions.invalidate_scope(scope);
                if session_id.is_some() {
                    count += 1;
                }
                agent.close_sessions(session_id.into_iter().collect()).await;
            }
        }
    ...
    pub fn channel_control_is_ambiguous(&self, channel_id: Uuid) -> bool {
        let mut scopes: HashSet<SessionScope> = self
            .session_owners
            .keys()
            .filter(|scope| scope.channel_id() == channel_id)
            .cloned()
        ...
        scopes.extend(
            self.task_map
                .values()
                .filter_map(|meta| meta.scope.as_ref())
                .filter(|scope| scope.channel_id() == channel_id)
                .cloned(),
        );
        scopes.len() > 1
    ...
    pub async fn switch_idle_agent_model(
        ...
        let has_channel_session = |agent: &OwnedAgent| {
            agent
                .runtime()
                .sessions
                .sessions
        ...
        {
            let capabilities = agent.runtime().model_capabilities.clone();
        ...
        let session_id = {
            ...
            runtime.desired_model_request_id = request_id;
            runtime.desired_model_pending_ack = false;
...
async fn create_session_and_apply_model(
    ...
    let goose_supported = agent.runtime().goose_system_prompt_supported;
    if is_goose && goose_supported != Some(false) {
        if let Some(prompt) = combined_system_prompt.as_deref() {
            match agent
                ...
                .await
            {
                Ok(_) => agent.runtime().goose_system_prompt_supported = Some(true),
                Err(AcpError::AgentError { code: -32601, .. }) => {
                    agent.runtime().goose_system_prompt_supported = Some(false);
                    tracing::warn!(
                        target: "pool::session",
    ...
    if agent.runtime().model_capabilities.is_none() {
        agent.runtime().model_capabilities = Some(AgentModelCapabilities {
    ...
    let (desired_model, pending_ack, desired_model_request_id) = {
        let mut runtime = agent.runtime();
        let desired_model = runtime.desired_model.clone();
        let pending_ack = if desired_model.is_some() {
            std::mem::take(&mut runtime.desired_model_pending_ack)
        } else {
            false
        };
        (
            desired_model,
            pending_ack,
            runtime.desired_model_request_id.clone(),
        )
    };
    let post_switch_snapshot: Option<serde_json::Value> = if let Some(ref desired) = desired_model {
                ...
                match apply_model_switch(&mut agent.acp, &resp.session_id, desired, &method).await?
                {
                    ModelSwitchOutcome::Applied(switch_result) => {
                            ...
                            .is_some_and(|v| !v.is_null())
                        {
                            ...
                            agent.runtime().model_capabilities = None;
                        }
                        ...
                        if pending_ack {
                            agent.acp.observe(
                                ...
                                serde_json::json!({
                                    ...
                                    "status": "switched",
                                    "modelId": desired,
                                    "requestId": desired_model_request_id,
                                }),
                            );
                        }
                        Some(switch_result)
    ...
    agent.acp.observe(
        ...
        serde_json::json!({
            ...
            "modelOverridden": agent.runtime().model_overridden && switch_succeeded,
...
async fn apply_startup_effort(
    ...
    let Some(value) = agent.runtime().startup_effort.clone() else {
        return Ok(None);
    };
...
pub async fn run_prompt_task(
    ...
    if ctx.memory_enabled {
            ...
            (&source, ctx.agent_owner_pubkey.as_ref())
        {
            ...
            let needs_core = {
                ...
                !runtime.sessions.sessions.contains_key(scope)
                    && !runtime.sessions.core_sections.contains_key(scope)
            };
            if needs_core {
    ...
    if let PromptSource::Channel(scope) = &source {
        let cid = scope.channel_id();
        let (is_new_channel_session, needs_canvas) = {
            let runtime = agent.runtime();
            let is_new = !runtime.sessions.sessions.contains_key(scope);
            (
                is_new,
                is_new && !runtime.sessions.canvas_sections.contains_key(scope),
            )
        };
    ...
    let agent_core: Option<String> = match &source {
        PromptSource::Channel(scope) => agent.runtime().sessions.core_sections.get(scope).cloned(),
        PromptSource::Heartbeat => None,
    };
    ...
    let (session_id, is_new_session) = match &source {
        PromptSource::Channel(scope) => {
            ...
            let existing_session = {
                let runtime = agent.runtime();
                runtime.sessions.sessions.get(scope).cloned()
            };
            if let Some(sid) = existing_session {
                ...
                .await
                {
                    Ok(sid) => {
                        ...
                        {
                            let mut runtime = agent.runtime();
                            runtime.sessions.sessions.insert(scope.clone(), sid.clone());
                            runtime
                                .sessions
                                .deliveries
        ...
        PromptSource::Heartbeat => {
            let existing = agent.runtime().sessions.heartbeat_session.clone();
            if let Some(sid) = existing {
                ...
                .await
                {
                    Ok(sid) => {
                        ...
                        agent.runtime().sessions.heartbeat_session = Some(sid.clone());
                        // Seed a zero usage baseline: buzz-acp spawned this session.
                        agent.acp.notify_session_spawned(&sid);
                        (sid, true)
                    }
    ...
    let mut standing_context_sent = match &source {
        ...
        PromptSource::Heartbeat => agent.runtime().sessions.heartbeat_standing_context_sent,
    };
            ...
            (&source, &ctx.initial_message)
        {
            ...
            match init_result {
                Ok(stop_reason) => {
                    ...
                    if !agent.has_system_prompt_support() {
                        agent.runtime().sessions.mark_scope_delivery_success(
                            scope.clone(),
                            true,
                            [],
                        );
            ...
            tokio::select! {
                ...
                mode = rx => {
                    ...
                    if let ControlSignal::SwitchModel { model_id, request_id } = &control_signal {
                        let mut runtime = agent.runtime();
                        runtime.desired_model = Some(model_id.clone());
                        runtime.model_overridden = true;
                        runtime.desired_model_request_id = request_id.clone();
                        ...
                        runtime.desired_model_pending_ack = true;
                    }
    ...
    match prompt_result {
        Ok(stop_reason) => {
            ...
            if let PromptSource::Channel(scope) = &source {
                ...
                agent.runtime().sessions.heartbeat_standing_context_sent = true;
            }
            ...
            let should_rotate = should_rotate || {
                ...
                if limit > 0 {
                    match &source {
                        ...
                        PromptSource::Heartbeat => {
                            agent.runtime().sessions.heartbeat_turn_count += 1;
                            agent.runtime().sessions.heartbeat_turn_count >= limit
                        }
                    }
                } else {
                    false
...
mod tests {
    ...
    async fn pool_grants_multiple_session_leases_from_one_process() {
        ...
        pool.set_session_concurrency(2);
        let first_key = SessionScope::Thread {
            channel_id: Uuid::new_v4(),
            root_event_id: "first".into(),
        };
        let second_key = SessionScope::Thread {
            channel_id: Uuid::new_v4(),
            root_event_id: "second".into(),
        };

        let first = pool.try_claim(Some(&first_key)).expect("first lease");
        let second = pool.try_claim(Some(&second_key)).expect("second lease");
        assert!(pool.try_claim(None).is_none(), "limit must be enforced");
        assert!(Arc::ptr_eq(&first.shared, &second.shared));

        pool.return_agent(first);
        assert!(
    ...
    async fn channel_invalidation_waits_for_its_last_active_thread() {
        ...
        let agent = OwnedAgent::new(0, acp, None, None, "test".into(), 2);
        let channel_id = Uuid::new_v4();
        let conversation = SessionScope::Thread {
            channel_id,
...
while IFS= read -r line; do
  ...
  if [ "$count" -eq 1 ]; then
    ...
    printf '%s\n' "{{\"jsonrpc\":\"2.0\",\"id\":$((count - 1)),\"result\":{{\"stopReason\":\"end_turn\"}}}}"
        ...
        let mut agent = OwnedAgent::new(0, acp, None, None, "legacy-test-agent".into(), 1);
        agent.runtime().sessions.heartbeat_session = Some("live-session".into());
        ...
        for turn in 1..=3 {
            ...
            assert_eq!(
                result
                    ...
                    .runtime()
                    .sessions
                    .heartbeat_standing_context_sent,
                turn >= 2,
  ...
  if [ "$count" -eq 1 ]; then
    ...
    printf '%s\n' "{{\"jsonrpc\":\"2.0\",\"id\":$((count - 1)),\"result\":{{\"stopReason\":\"end_turn\"}}}}"
        ...
        for turn in 1..=3 {
            ...
            let runtime = result.agent.runtime();
            let delivery = &runtime.sessions.deliveries[&conv(channel_id)];
            assert_eq!(
...
printf '%s\n' '{{"jsonrpc":"2.0","id":0,"result":{{"stopReason":"end_turn"}}}}'"#
    ...
    fn delivery_state_is_cleared_on_rotation_and_restarts_empty() {
        ...
        assert!(!state.invalidate_channel(&channel).is_empty());
    ...
    fn invalidate_scope_leaves_sibling_thread_untouched() {
        ...
        assert!(s.invalidate_scope(&ta).is_some());
    ...
    fn invalidate_channel_clears_every_thread_scope() {
        ...
        assert_eq!(cleared.len(), 3, "all three ch scopes had sessions");
    ...
    fn test_invalidate_channel_returns_retired_session() {
        let (mut s, ch_a, ch_b) = make_state();
        assert!(!s.invalidate_channel(&ch_a).is_empty());
        assert!(!s.sessions.contains_key(&conv(ch_a)));
        assert!(!s.turn_counts.contains_key(&conv(ch_a)));
        assert!(!s.core_sections.contains_key(&conv(ch_a)));
        assert!(!s.has_channel_state(&ch_a));
    ...
    async fn test_send_prompt_result_clears_steer_rx_on_early_return() {
        ...
        let mut agent = OwnedAgent::new(0, acp, None, None, "unknown".into(), 2);
    ...
    async fn test_send_prompt_result_is_noop_when_steer_rx_already_consumed() {
        ...
        .await
        .expect("failed to spawn test agent");
        let agent = OwnedAgent::new(0, acp, None, None, "unknown".into(), 2);
    ...
    fn thread_conv(channel_id: Uuid, root: &str) -> SessionScope {
        SessionScope::Thread {
            channel_id,
    ...
    fn test_session_state_keeps_threads_in_one_channel_independent() {
        ...
        assert_eq!(s.invalidate_scope(&thread_a).as_deref(), Some("sess-a"));
  ...
  printf '%s\n' '{{"jsonrpc":"2.0","id":0,"result":{{"stopReason":"end_turn"}}}}'
        ...
        let acp = AcpClient::spawn("bash", &["-c".into(), script], &[], false)
            .await
            .expect("spawn wire-capture ACP");
        let agent = OwnedAgent::new(0, acp, None, None, "boundary-test-agent".into(), 1);
...
mod startup_effort_tests {
    ...
    fn effort_agent(acp: AcpClient, startup_effort: Option<&str>) -> OwnedAgent {
        OwnedAgent::new(
            ...
            acp,
            None,
            startup_effort.map(str::to_string),
            "effort-test-agent".into(),
            2,
        )
    }
...
mod model_switch_tests {
    ...
    fn switching_agent(acp: AcpClient, desired_model: &str) -> OwnedAgent {
        let agent = OwnedAgent::new(
            0,
            acp,
            Some(desired_model.to_string()),
            None,
            "switch-test-agent".into(),
            2,
        );
        agent.runtime().model_overridden = true;
        agent
    }
...
while IFS= read -r line; do
  ...
  if [ "$count" -eq 1 ]; then
    ...
    async fn session_new_sends_policy_specific_base_and_scope_specific_title() {
        ...
        for (policy, scope, name, channel_type, title) in [
            ...
            for (version, include_base) in [(1, true), (2, true), (1, false), (2, false)] {
                ...
                agent.runtime().desired_model = None;
    ...
    async fn idle_channel_switch_preserves_all_sibling_sessions_and_model() {
        ...
        let original_sessions = agent.runtime().sessions.sessions.clone();
        let mut pool = AgentPool::from_slots(vec![Some(agent)]);
        assert_eq!(
        ...
        assert_eq!(agent.runtime().desired_model_request_id, None);
        assert_eq!(agent.runtime().sessions.sessions, original_sessions);
        ...
        assert!(!agent.runtime().sessions.sessions.contains_key(&scopes[0]));
    ...
    async fn test_applied_switch_refreshes_capabilities_from_post_switch_snapshot() {
        ...
        // terminal instead of leaving the Desktop to infer success from silence.
        {
            let mut runtime = agent.runtime();
            runtime.desired_model_pending_ack = true;
            runtime.desired_model_request_id = Some("req-busy-1".into());
        }
        let obs = observer::ObserverHandle::in_process();
        agent.acp.set_observer(Some(obs.clone()), 0);
        ...
        assert!(
            !agent.runtime().desired_model_pending_ack,
    ...
    async fn test_busy_path_rejection_emits_only_failure_and_consumes_pending_ack() {
        ...
        let mut agent = switching_agent(acp, "model-b");
        {
            let mut runtime = agent.runtime();
            runtime.desired_model_pending_ack = true;
            runtime.desired_model_request_id = Some("req-busy-reject".into());
        }
        let obs = observer::ObserverHandle::in_process();
        agent.acp.set_observer(Some(obs.clone()), 0);
    ...
    async fn test_applied_switch_without_options_drops_capabilities() {
        ...
        assert!(
            agent.runtime().model_capabilities.is_none(),
  ...
  if [ "$count" -eq 1 ]; then
    ...
    fn switching_agent_with_effort(
        ...
        let agent = OwnedAgent::new(
            ...
            acp,
            Some(desired_model.to_string()),
            Some(startup_effort.to_string()),
            "switch-effort-test-agent".into(),
            2,
        );
        agent.runtime().model_overridden = true;
        agent
